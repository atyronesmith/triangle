...
const QUOTES = [
  ...
  { s: 4, q: "If AI speeds up 40% of your workflow by 3x, Amdahl says your total speedup is 1.36x, not 3x. The serial bottleneck is the constraint that matters.", a: "Systems engineer" },
  ...
  // More negative for variety
  { s: 1, q: "We optimized for the metric and destroyed the thing the metric was supposed to measure.", a: "Goodhart's Law in practice" },
  ...
  { s: 3, q: "Our AI adoption is at 93%. Our customer satisfaction is at a five-year low. Coincidence is not the word I'd use.", a: "Customer success lead" },
  { s: 2, q: "Management sees the output numbers and thinks we're thriving. The team sees the quality numbers and knows we're drowning.", a: "Anonymous engineer" },

  // More positive for variety
  ...
  { s: 4, q: "AI adoption without process adaptation is just faster chaos.", a: "Process improvement coach" },
  ...
  { s: 4, q: "As the unit price of intelligence falls, firms redesign their architectures to consume dramatically more of it.", a: "Zhang & Zhang, 2026" },
  ...
  { s: 4, q: "We're optimizing for throughput in a system that's constrained by comprehension.", a: "Systems thinker" },
]
...
function stateToSentiment(state) {
  ...
  // Weighted score: quality (40%), inverse debt (30%), morale (30%)
  const qualScore = q / 10                    // 0-10
  const debtScore = (100 - debt) / 10         // 0-10 (inverted)
  const moraleScore = morale / 10             // 0-10

  return Math.round(qualScore * 0.4 + debtScore * 0.3 + moraleScore * 0.3)
}

/**
 * Pick a random quote near the target sentiment.
 * Allows ±2 range for variety.
 */
...
export function updateQuoteSentiment(state) {
  // Only update on the timer, but store state for next tick
  if (!quoteEl) return
...
function showQuote(state) {
  ...
  quoteEl.innerHTML = `<span class="quote-text">"${quote.q}"</span><span class="quote-attr">— ${quote.a}</span>`
  quoteEl.className = 'quote-content' + (sentiment <= 3 ? ' quote-neg' : sentiment >= 7 ? ' quote-pos' : '')
}
